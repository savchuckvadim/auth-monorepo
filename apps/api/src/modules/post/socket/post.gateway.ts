...
export class PostGateway implements OnGatewayConnection, OnGatewayDisconnect {
    ...
    handleConnection(client: Socket) {
        ...
        void client.join('feed:subscribers');
    }
    ...
    handlePostCreated(event: PostCreatedEvent) {
        ...
        this.server.to('feed:subscribers').emit('post:created', event.post);

        // Также отправляем держателю поста (если он онлайн)
        this.server
            .to(`user:${event.post.userId}`)
            .emit('post:created', event.post);
    }

    @OnEvent(PostEvent.UPDATED)
    handlePostUpdated(event: PostUpdatedEvent) {
        // Отправляем всем подписчикам feed
        this.server.to('feed:subscribers').emit('post:updated', event.post);

        // Отправляем автору
        this.server
            .to(`user:${event.post.userId}`)
            .emit('post:updated', event.post);
    }
    ...
    handlePostDeleted(event: PostDeletedEvent) {
        ...
        this.server
            .to('feed:subscribers')
            .emit('post:deleted', { postId: event.postId });
    }

    @OnEvent(PostEvent.LIKED)
    handlePostLiked(event: PostLikedEvent) {
        // Отправляем всем подписчикам feed
        this.server.to('feed:subscribers').emit('post:liked', {
            postId: event.postId,
            userId: event.userId,
            isLike: event.isLike,
        });
