...
export abstract class PostRepository {
    ...
    abstract getFeed(currentUserId: string, cursor?: string, limit?: number): Promise<{ posts: FullPost[]; nextCursor?: string; hasNext: boolean }>;
    abstract getByUserId(userId: string, currentUserId?: string, cursor?: string, limit?: number): Promise<{ posts: FullPost[]; nextCursor?: string; hasNext: boolean }>;
    abstract getMyReposts(userId: string, cursor?: string, limit?: number): Promise<{ posts: FullPost[]; nextCursor?: string; hasNext: boolean }>;
    ...
    abstract repost(postId: string, userId: string, data?: RepostDto): Promise<FullPost>;
